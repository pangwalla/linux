...
static int nt36523_write(struct nvt_ts_data * ts, unsigned int reg, unsigned int val)
{
  return regmap_write(ts->regmap, reg, val);
}

static int nt36523_read(struct nvt_ts_data * ts, unsigned int reg, unsigned int *val)
{
  return regmap_read(ts->regmap, reg, val);
}
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	ts->regmap = devm_regmap_init_spi(client, &nt36523_regmap_config);
	if (IS_ERR(ts->regmap))
		return dev_err_probe(&client->dev, PTR_ERR(ts->regmap), "failed to init regmap\n");
